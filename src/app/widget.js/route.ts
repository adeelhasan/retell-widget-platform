...
(function() {
  ...
  // Site Builder Initialization - looks for [data-retell-widget] divs
  function initializeSiteBuilderWidget() {
    console.log('🏗️ Retell Widget (Site Builder Mode): Initializing...');
    injectStyles();

    // Find the container div with data-retell-widget attribute
    const container = document.querySelector('[data-retell-widget]');

    if (!container) {
      console.error('❌ Retell Widget: No [data-retell-widget] div found on page.');
      console.log('💡 Add this to your page: <div data-retell-widget="your-widget-id"></div>');
      return;
    }

    const widgetId = container.getAttribute('data-retell-widget');

    if (!widgetId) {
      console.error('❌ Retell Widget: [data-retell-widget] attribute is empty.');
      return;
    }

    console.log('✅ Found widget container:', { widgetId });

    // Check for optional data attributes on container
    const buttonText = container.getAttribute('data-button-text');

    // Determine baseUrl from the script source
    ...
    if (scripts.length > 0) {
      try {
        ...
        console.warn('Could not determine baseUrl from script src, using current origin');
      }
    }

    console.log('🔧 Initializing widget with config:', { widgetId, buttonText, baseUrl });

    // Create widget instance directly in the designated container
    ...
    console.log('✅ Retell Widget (Site Builder Mode) initialized successfully');
  }
  
  // Initialize when DOM is ready
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeSiteBuilderWidget);
  } else {
    initializeSiteBuilderWidget();
  }
...
`;
  ...
  return new NextResponse(widgetScript, {
    headers: {
      ...
      'Cache-Control': 'public, max-age=3600', // Cache for 1 hour (can be cached longer than widget-simple)
